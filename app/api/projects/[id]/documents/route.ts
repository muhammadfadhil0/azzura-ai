...
export async function POST(
  ...
  if (file.size > MAX_FILE_BYTES) return jsonError('File exceeds 25MB limit', 400)
  ...
  if ((count ?? 0) >= MAX_DOCS_PER_PROJECT) {
    return jsonError(`Maximum ${MAX_DOCS_PER_PROJECT} documents per project`, 400)
  }
  ...
  if (insertErr || !docRow) {
    console.error('Failed to insert project document', insertErr?.message)
    return jsonError(insertErr?.message ?? 'Failed to create document', 500)
  }
  const docId = docRow.id as string
  const path = buildProjectDocumentPath(user.id, projectId, docId, mimeType)
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        send({ type: 'doc.created', id: docId, name: fileName, mimeType, sizeBytes })

        await uploadDocumentFile(admin, path, new Blob([arrayBuffer], { type: mimeType }), mimeType)
        await admin.from('documents').update({ storage_path: path, status: 'parsing' }).eq('id', docId)
